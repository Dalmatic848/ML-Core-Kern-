...
|-----|-------------|---------|-----------------|-------|--------|
| run_v3 ДС | ResNet18 single | 5 | **0.488** | 6 | baseline |
| run_v3 УФ | ResNet18 single | 5 | **0.418** | 7 | baseline |
...
- **Дообучить УФ ResNet50** — `./run.sh run_rn50_v2 resnet50` (полный прогон, ~60 эпох). УФ в run_rn50_v1 прервана на 4 эпохах
...
- **Чистый ResNet18 dual** — `python3 train.py run_v5 --mode dual` (run_v4 имеет best epoch=2 на неполном датасете)

### Приоритет: средний

- **Алевролит F1≈0** — focal γ=3–4 для этого класса, или merge с Аргиллитом (6→5 классов). Класс редкий и визуально близок к Перес_тёмное
- **Перес_светлое recall=0.10** — модель почти не предсказывает этот класс на test при высокой precision: проблема генерализации
- **Сравнение ResNet18 vs ResNet50** — корректное только после полного обучения обоих

### Приоритет: низкий

- **Нормализация by-well** — mean/std из конкретной скважины перед подачей
- **Больше скважин** — 16 скважин мало для надёжного test-gap

---

## История изменений

### v6 (текущая) — 2026-06-04
