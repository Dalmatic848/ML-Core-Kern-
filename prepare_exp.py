...
# ──────────────────────────────────────────────────────────────────────────────
# Мягкие метки — загружаем из soft_labels_config.json
# Ключи конфига содержат пробелы (как в CSV). Здесь нормализуем в underscores.
# ──────────────────────────────────────────────────────────────────────────────
_SOFT_CONFIG_PATH = Path(__file__).parent / 'soft_labels_config.json'

def _load_soft_config() -> tuple:
    """Загружает конфиг и возвращает (BASE_CLASSES, SOFT_LABEL_MAP_with_underscores)."""
    with open(_SOFT_CONFIG_PATH, encoding='utf-8') as f:
        cfg_data = json.load(f)
    base_classes = cfg_data['_base_classes']
    # нормализуем ключи: пробелы → подчёркивания (формат имён тайлов)
    soft_map: Dict[str, Optional[list]] = {}
    for mineral_raw, vec in cfg_data['minerals'].items():
        key = mineral_raw.strip().replace(' ', '_')
        soft_map[key] = vec
    return base_classes, soft_map

BASE_CLASSES, SOFT_LABEL_MAP = _load_soft_config()
